...
if start_idx != -1 and end_idx != -1:
    ...
    if (g_sdlWindow) {
        ...
        for (GLuint fbo = 1; fbo <= 5; fbo++) {
            if (glIsFramebuffer(fbo)) {
                ...
                glBlitFramebuffer(0, 0, 1024, 768, 0, 768, 1024, 0, GL_COLOR_BUFFER_BIT, GL_LINEAR);
                GLenum err = glGetError();
                if (frames % 60 == 0) { printf("Blit FBO %u status: %04X\\n", fbo, err); fflush(stdout); }
            }
        }
    ...
    print("Present patched for FBO blit logging.")
else:
