...
struct LinuxStatUpgradeInfo
{
  float increment;
  vector<NDb::StatUpgrade> upgrades;

  LinuxStatUpgradeInfo() : increment(0.0f) {}
};
typedef vector<LinuxStatUpgradeInfo> TLinuxUnitStatUpgrades;
static map<const PFBaseUnit*, TLinuxUnitStatUpgrades> g_linuxUnitStatUpgrades;
...
static void LinuxUnregisterBootstrapUnit(PFBaseUnit* unit)
{
  ...
  g_linuxUnitStatUpgrades.erase(unit);
}
...
static TLinuxUnitStatUpgrades& GetLinuxUnitStatUpgrades(const PFBaseUnit* unit)
{
  TLinuxUnitStatUpgrades& upgrades = g_linuxUnitStatUpgrades[unit];
  if (upgrades.size() < NDb::KnownEnum<NDb::EStat>::SizeOf())
    upgrades.resize(NDb::KnownEnum<NDb::EStat>::SizeOf());
  return upgrades;
}

static void LinuxClearUnitStats(const PFBaseUnit* unit)
{
  GetLinuxUnitStats(unit).assign(NDb::KnownEnum<NDb::EStat>::SizeOf(), StatValueWithModifiers());
  GetLinuxUnitStatUpgrades(unit).assign(NDb::KnownEnum<NDb::EStat>::SizeOf(), LinuxStatUpgradeInfo());
}

struct LinuxStatUpgradeLevelLess
{
...
struct LinuxStatFormulaParams : public IMiscFormulaPars
{
  LinuxStatFormulaParams(NDb::Ptr<NDb::UnitConstantsContainer> const& pConstants) : pConstants(pConstants) {}
  ...
  virtual bool Roll(float) const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return false; }
  ...
  virtual float GetCooldown() const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return 0.0f; }
  ...
  virtual const IUnitFormulaPars* GetObjectOwner() const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return 0; }
  virtual const IMiscFormulaPars* GetObjectParent() const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return 0; }
  virtual const IUnitFormulaPars* GetObject(char const*) const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return 0; }
  virtual float GetVariable(const char*) const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return 0.0f; }
  virtual CVec2 GetTargetPos() const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return VNULL2; }
  virtual int GetRandom(int, int) const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return 0; }
  virtual float GetConstant(const char* name, IUnitFormulaPars const* pSender, IUnitFormulaPars const* pReceiver) const;
  virtual float GetAbilityScale(bool, float, EAbilityScaleMode, float, float, bool = true) const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return 0.0f; }
  virtual bool GetSmartRoll(float, int, int, const IUnitFormulaPars*, const IUnitFormulaPars*) const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return false; }
  virtual int GetSmartRandom(int, float, const IUnitFormulaPars*, const IUnitFormulaPars*) const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return 0; }
  virtual float GetRefineAbilityScale(float, float) const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return 0.0f; }
  virtual int GetAlternativeTargetIndex() const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return -1; }
  virtual float GetTerrainPart(int) const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return -1.0f; }
  ...
  virtual float GetStatusDispellPriority(const IUnitFormulaPars*, bool) const { return 0.0f; }
  virtual CVec2 GetVectorVariable(const char*) const { return VNULL2; }
  virtual int GetTerrianTypeUnderCursor() const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return -1; }
  virtual int GetNatureTypeInPos(CVec2) const { NI_ALWAYS_ASSERT("Only const variables may be used in Stats"); return -1; }

  NDb::Ptr<NDb::UnitConstantsContainer> pConstants;
};

float LinuxStatFormulaParams::GetConstant(const char* name, IUnitFormulaPars const* pSender, IUnitFormulaPars const* pReceiver) const
{
  if (!pConstants)
    return 0.0f;

  const vector<NDb::Ptr<NDb::UnitConstant> >& glob = pConstants->vars;
  for (vector<NDb::Ptr<NDb::UnitConstant> >::const_iterator it = glob.begin(), end = glob.end(); it != end; ++it)
  {
    if ((*it) && (*it)->name == name)
      return (*it)->var(pSender, pReceiver, this, 0.0f);
  }

  NI_DATA_VERIFY(false, NStr::StrFmt("Constant '%s' not found in globalConstants!", name), return 0.0f;);
  return 0.0f;
}

static void LinuxLoadStatsContainerForUnit(
  PFBaseUnit* unit,
  const NDb::StatsContainer* statsContainer,
  const NDb::Ptr<NDb::UnitConstantsContainer>& constants,
  const NDb::Ptr<NDb::StatsEnhancersContainer>& statsEnhancers)
{
  if (!unit || !statsContainer)
    return;

  LinuxStatFormulaParams params(constants);
  TLinuxUnitStats& stats = GetLinuxUnitStats(unit);
  TLinuxUnitStatUpgrades& upgrades = GetLinuxUnitStatUpgrades(unit);
  const int statCount = NDb::KnownEnum<NDb::EStat>::SizeOf();

  for (vector<NDb::UnitStat>::const_iterator it = statsContainer->stats.begin(), end = statsContainer->stats.end(); it != end; ++it)
  {
    const int statIndex = static_cast<int>(it->statId);
    if (statIndex < 0 || statIndex >= statCount)
      continue;

    float value = it->value(0, 0, &params);
    float increment = it->increment(0, 0, &params);

    if (statsEnhancers != 0)
    {
      for (vector<NDb::StatEnhancer>::const_iterator enhancer = statsEnhancers->statsEnhancers.begin(), enhancerEnd = statsEnhancers->statsEnhancers.end(); enhancer != enhancerEnd; ++enhancer)
      {
    ...
    StatValueWithModifiers& stat = stats[statIndex];
    stat = StatValueWithModifiers();
    ...
    case NDb::STAT_CRITCHANCEBONUS:
      break;
    default:
      stat.AddModifier(0.0f, 0.0f, std::numeric_limits<int>::max(), true, NDb::STATCONSTRAINT_LOWERLIMIT);
      break;
    }

    if (increment != 0.0f || !it->levelUpgrades.empty())
    {
      LinuxStatUpgradeInfo& upgrade = upgrades[statIndex];
      upgrade.increment = increment;
      upgrade.upgrades = it->levelUpgrades;
      sort(upgrade.upgrades.begin(), upgrade.upgrades.end(), LinuxStatUpgradeLevelLess());
    }
  }
}

static void LinuxLoadUnitStats(
  PFBaseUnit* unit,
  PFWorld* world,
  const NDb::Unit* unitDesc,
  NDb::EUnitType unitType,
  const NDb::Ptr<NDb::StatsEnhancersContainer>& statsEnhancers = 0)
{
  if (!unit)
    return;

  LinuxClearUnitStats(unit);

  NDb::Ptr<NDb::UnitConstantsContainer> constants;
  const NDb::StatsContainer* defaultStats = 0;
  if (world && world->GetAIWorld())
  {
    NDb::AILogicParameters const& aiParams = world->GetAIWorld()->GetAIParameters();
    constants = aiParams.globalConstants;
    defaultStats = world->GetAIWorld()->GetUnitParameters(unitType).defaultStats;
  }

  LinuxLoadStatsContainerForUnit(unit, defaultStats, constants, statsEnhancers);
  LinuxLoadStatsContainerForUnit(unit, unitDesc ? unitDesc->stats.GetPtr() : 0, constants, statsEnhancers);
}

static float LinuxGetStatUpgradeIncrement(const LinuxStatUpgradeInfo& upgrade, const unsigned fromLevel, const unsigned count)
{
  if (upgrade.upgrades.empty())
    return upgrade.increment * count;

  float increment = 0.0f;
  float upgradeIncrement = 0.0f;
  vector<NDb::StatUpgrade>::const_iterator it = upgrade.upgrades.begin();
  for (unsigned level = fromLevel + 1U, cap = fromLevel + count; level <= cap; ++level)
  {
    for (; it != upgrade.upgrades.end(); ++it)
    {
      if (static_cast<unsigned>(it->level) > level)
        break;
      upgradeIncrement = it->increment;
    }
...
static void LinuxUpgradeUnitStats(PFBaseUnit* unit, const int oldLevel, const int count, const float fraction = 1.0f)
{
  if (!unit || count <= 0)
    return;
  ...
  TLinuxUnitStats& stats = GetLinuxUnitStats(unit);
  TLinuxUnitStatUpgrades& upgrades = GetLinuxUnitStatUpgrades(unit);
  const int statCount = Min(static_cast<int>(stats.size()), static_cast<int>(upgrades.size()));
  for (int i = 0; i < statCount; ++i)
  {
    const float increment = LinuxGetStatUpgradeIncrement(upgrades[i], static_cast<unsigned>(Max(oldLevel, 0)), static_cast<unsigned>(count)) * fraction;
    if (increment != 0.0f)
      stats[i].SetCoreValue(stats[i].GetCoreValue() + increment);
  }
}

static float LinuxClampStatFormula(float value, float minValue, float maxValue)
{
  return Min(Max(value, minValue), maxValue);
}

static void LinuxInitializeUnitGeometry(PFBaseUnit* unit, PFWorld* world, const NDb::Unit* unitDesc)
{
  if (!unit)
    return;

  float tileSize = 1.0f;
  if (world && world->GetTileMap() && world->GetTileMap()->GetTileSize() > EPS_VALUE)
    tileSize = world->GetTileMap()->GetTileSize();

  const float dbSize = unitDesc ? unitDesc->size : 0.0f;
  const float objectSizeSource = dbSize > EPS_VALUE ? dbSize : tileSize;
  const int sizeInTiles = Max(1, static_cast<int>(ceil(objectSizeSource / tileSize)));

  const float dbDynamicSize = unitDesc ? unitDesc->sizeDynamic : -1.0f;
  const int dynamicSizeInTiles =
    dbDynamicSize > EPS_VALUE
      ? Max(1, static_cast<int>(ceil(dbDynamicSize / tileSize)))
      : sizeInTiles;

  unit->SetObjectSizes(static_cast<float>(sizeInTiles) * tileSize, sizeInTiles, dynamicSizeInTiles);
}
...
{
  if (!pAIParams || pAIParams->statsBudget.IsEmpty())
    return 0;
  ...
  , attackSectorsAngle(0.0f)
{
  ...
  if (_dbUnitDesc)
  {
    baseUnitTargetingParams = _dbUnitDesc->targetingParams;
    unitTargetingParams = _dbUnitDesc->targetingParams;
...
bool PFBaseUnit::HasAtackAbility() const { return pAttackAbility != 0; }
...
void PFBaseUnit::Initialize(InitData const& data)
{
  ...
  LinuxLoadUnitStats(this, GetWorld(), dbUnitDesc, data.type);
  if (dbUnitDesc && dbUnitDesc->attackAbility && !pAttackAbility)
    pAttackAbility = new PFBaseAttackData(CPtr<PFBaseUnit>(this), dbUnitDesc->attackAbility);
...
{
  const float life = GetStatValue(NDb::STAT_LIFE);
  const float energy = GetStatValue(NDb::STAT_ENERGY);
  const float strength = GetStatValue(NDb::STAT_STRENGTH);
  const float intellect = GetStatValue(NDb::STAT_INTELLECT);
  const float dexterity = GetStatValue(NDb::STAT_DEXTERITY);
  const float attackSpeed = GetStatValue(NDb::STAT_ATTACKSPEED);
  const float stamina = GetStatValue(NDb::STAT_STAMINA);
  const float will = GetStatValue(NDb::STAT_WILL);

  switch (stat)
  {
  case NDb::DERIVATIVESTAT_LIFEREGENTOTAL:
    return GetStatValue(NDb::STAT_LIFEREGENERATIONABSOLUTE) + GetStatValue(NDb::STAT_LIFEREGENERATION) * life * 0.01f;
  case NDb::DERIVATIVESTAT_ENERGYREGENTOTAL:
    return GetStatValue(NDb::STAT_ENERGYREGENERATIONABSOLUTE) + GetStatValue(NDb::STAT_ENERGYREGENERATION) * energy * 0.01f;
  case NDb::DERIVATIVESTAT_DAMAGEMIN:
    return GetStatValue(NDb::STAT_BASEATTACK) * (GetNativeAttackIndex() ? intellect : strength) * 0.9f + GetVariableValue("DamageSummand");
  case NDb::DERIVATIVESTAT_DAMAGEMAX:
    return GetStatValue(NDb::STAT_BASEATTACK) * (GetNativeAttackIndex() ? intellect : strength) * 1.1f + GetVariableValue("DamageSummand");
  case NDb::DERIVATIVESTAT_DEXTERITYDAMAGEMULT:
    return 1.0f;
  case NDb::DERIVATIVESTAT_MOVESPEEDMPS:
    {
      const float moveSpeed = GetStatValue(NDb::STAT_MOVESPEED);
      const float moveSpeedMult = GetVariableValue("MoveSpeedMult");
      return moveSpeed / (moveSpeed + 172.0f) * 30.0f * (moveSpeedMult == 0.0f ? 1.0f : moveSpeedMult);
    }
  case NDb::DERIVATIVESTAT_CRITCHANCE:
    {
      const float adjustedDexterity = dexterity - Max(0.0f, dexterity - 500.0f) * 0.25f;
      return LinuxClampStatFormula(90.9f * adjustedDexterity / (adjustedDexterity + 128.0f) - 28.0f + GetStatValue(NDb::STAT_CRITCHANCEBONUS), 0.0f, 100.0f);
    }
  case NDb::DERIVATIVESTAT_ATTACKSPERSECOND:
    {
      const float explicitAttacksPerSecond = GetVariableValue("ExplicitAttacksPerSecond");
      return Max(explicitAttacksPerSecond > 0.0f ? explicitAttacksPerSecond : Min(attackSpeed * 0.00365f + 0.49f, 2.0f), 0.01f);
    }
  case NDb::DERIVATIVESTAT_BODYDEFENCE:
    return 0.00536f * (stamina + will * 0.15f) - 0.2f;
  case NDb::DERIVATIVESTAT_SPIRITDEFENCE:
    return 0.00536f * (will + stamina * 0.15f) - 0.2f;
  case NDb::DERIVATIVESTAT_PENETRATIONFROMATTACKSPEED:
    {
      const float adjustedAttackSpeed = attackSpeed - Max(0.0f, attackSpeed - 500.0f) * 0.25f;
      const float attacksPerSecond = LinuxClampStatFormula(attackSpeed * 0.00365f + 0.49f, 0.702f, 2.0f);
      return 0.00536f * adjustedAttackSpeed - 0.2f - (1.0f - 0.6667f / attacksPerSecond);
    }
  case NDb::DERIVATIVESTAT_PENETRATIONFROMDEXTERITY:
    {
      const float adjustedDexterity = dexterity - Max(0.0f, dexterity - 500.0f) * 0.25f;
      const float critChance = LinuxClampStatFormula(90.9f * adjustedDexterity / (adjustedDexterity + 128.0f) - 28.0f, 0.0f, 100.0f);
      return 0.00536f * adjustedDexterity - 0.2f - critChance / (100.0f + critChance);
    }
  case NDb::DERIVATIVESTAT_PENETRATIONTOTAL:
    return Max(GetStatValue(NDb::DERIVATIVESTAT_PENETRATIONFROMATTACKSPEED) + GetStatValue(NDb::DERIVATIVESTAT_PENETRATIONFROMDEXTERITY), 0.0f);
  default:
    break;
  }

  const int index = static_cast<int>(stat);
  if (!derivativeStatsContainer || index < 0 || index >= derivativeStatsContainer->derivativeStats.size())
    return 0.0f;
  return derivativeStatsContainer->derivativeStats[index](this, 0, 0);
}
...
{
  return dbUnitDesc && dbUnitDesc->nativeDefence != NDb::DAMAGETYPE_MATERIAL ? 1 : 0;
}
int PFBaseUnit::GetNativeAttackIndex() const
{
  return dbUnitDesc && dbUnitDesc->nativeAttack != NDb::DAMAGETYPE_MATERIAL ? 1 : 0;
}
...
CPtr<PFBaseUnit> PFBaseUnit::FindTarget(float fRange, bool, int, bool checkForbid)
{
  ...
  {
    PFBaseUnit* candidate = it->GetPtr();
    if (!candidate || candidate == this || candidate->GetWorld() != GetWorld())
      continue;
    if (candidate->IsDead() || !candidate->IsVulnerable())
      continue;
    if (candidate->GetFaction() == GetFaction())
      continue;
    if (candidate->GetFaction() == NDb::FACTION_NEUTRAL && GetFaction() != NDb::FACTION_NEUTRAL)
      continue;
    if (!CanAttackTarget(candidate) || !CanSelectTarget(candidate, false))
      continue;
    if (!IsTargetInRange(candidate, range))
      continue;

    const float distance2 = fabs2(candidate->GetPosition().AsVec2D() - origin);
...
bool PFBaseUnit::Step(float dtInSeconds)
{
  ...
  {
    if (CanAttackTarget(pCurrentTarget))
    {
      if (IsTargetInAttackRange(pCurrentTarget, true))
      {
        if (IsReadyToAttack())
          DoAttack(false);
      }
      else if (!CheckFlagType(NDb::UNITFLAGTYPE_FORBIDMOVE))
      {
        PFBaseMovingUnit* movingUnit = dynamic_cast<PFBaseMovingUnit*>(this);
        if (movingUnit)
        {
          float chaseStopDistance = GetAttackRange();
          if (pAttackAbility)
          {
            const float abilityRange = pAttackAbility->GetUseRange(pCurrentTarget.GetPtr());
            if (abilityRange > EPS_VALUE)
              chaseStopDistance = abilityRange;
          }
          movingUnit->MoveTo(pCurrentTarget.GetPtr(), Max(0.0f, chaseStopDistance * 0.9f));
        }
      }
    }
  }
...
void PFBaseUnit::OnUnitDie(CPtr<PFBaseUnit> pKiller, int flags, PFBaseUnitDamageDesc const* pDamageDesc)
{
  health = 0.0f;
  ...
  PFAIWorld* aiWorld = GetWorld() ? GetWorld()->GetAIWorld() : 0;
  const bool deathDeferred = (flags & UNITDIEFLAGS_DEFERREDDEATH) != 0;
  const bool rewardsForbidden = (flags & UNITDIEFLAGS_FORBIDREWARDS) != 0;

  if (!deathDeferred && aiWorld && ringField.isLinked())
    aiWorld->UnregisterObjectOrUnit(this);

  if (!rewardsForbidden && aiWorld)
  {
    if (IsValid(pKiller))
      aiWorld->AwardKillers(this, pKiller);

    if ((flags & UNITDIEFLAGS_UNSUMMON) == 0)
      aiWorld->SendUnitDieNotification(this, pKiller, pDamageDesc);
  }
...
void PFBaseUnit::CleanupAfterDeath(bool fullCleanup)
{
  if (IsValid(pAttackAbility))
  {
    pAttackAbility->Cancel();
...
bool PFBaseUnit::DoAttack(bool allowAllies)
{
  ...
  if (!IsUnitValid(pCurrentTarget) || !IsReadyToAttack() || !pAttackAbility)
    return false;
...
void PFBaseUnit::UpgradeStats(int count, float)
{
  if (count <= 0)
    return;

  const float healthPercent = GetHealthPercent();
  const float energyPercent = GetManaPercent();
  LinuxUpgradeUnitStats(this, naftaLevel, count);
...
void PFBaseUnit::UpgradeHeroStats(const int count, const float, const float fraction)
{
  if (count <= 0 || fraction <= 0.0f)
    return;

  const float healthPercent = GetHealthPercent();
  const float energyPercent = GetManaPercent();
  LinuxUpgradeUnitStats(this, naftaLevel, count, fraction);
  maxHealth = Max(GetStatValue(NDb::STAT_LIFE), 1.0f);
  maxEnergy = Max(GetStatValue(NDb::STAT_ENERGY), 0.0f);
  health = maxHealth * healthPercent;
  energy = maxEnergy * energyPercent;
}
void PFBaseUnit::DoLevelups(int count, float)
{
  if (count <= 0)
    return;
...
{
  return dbUnitDesc && dbUnitDesc->interaction ? dbUnitDesc->interaction->externalAbility : 0;
}
bool PFBaseUnit::CheckUseLimitations(PFBaseUnit const* pUnit, NDb::CastLimitation const*& castLimitation) const
{
  castLimitation = 0;
  if (!GetExternalAbility() || !pUnit)
    return false;

  CObj<PFAbilityData> pAbilityData =
    new PFAbilityData(const_cast<PFBaseUnit*>(pUnit), GetExternalAbility(), NDb::ABILITYTYPEID_SPECIAL, false);
...
{
  NDb::CastLimitation const* limitation = 0;
  return pUnit &&
    !pUnit->IsMounted() &&
    !pUnit->CheckFlagType(NDb::UNITFLAGTYPE_FORBIDINTERACT) &&
    CheckUseLimitations(pUnit, limitation);
}
bool PFBaseUnit::IsUsableBy(PFBaseUnit const* pUnit) const
{
  return pUnit &&
    dbUnitDesc &&
    dbUnitDesc->interaction &&
    dbUnitDesc->interaction->externalAbility &&
    (GetFactionMask(dbUnitDesc->interaction->allowedFactions) & (1 << pUnit->GetFaction()));
}
CObj<PFAbilityInstance> PFBaseUnit::Use(PFBaseUnit* pUser)
{
  if (!pUser || !CanBeUsedBy(pUser))
    return 0;

  CObj<PFAbilityInstance> pAbilityInstance =
    pUser->UseExternalAbility(GetExternalAbility(), Target(this));
...
{
  if (!GetExternalAbility() || !pUser)
    return 0.0f;

  CObj<PFAbilityData> pAbilityData =
    new PFAbilityData(pUser, GetExternalAbility(), NDb::ABILITYTYPEID_SPECIAL, false);
  return pAbilityData->GetUseRange(this);
}
