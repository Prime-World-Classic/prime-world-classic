...
bool PFBaseCreep::CanLevelUpNow() const { return !IsInCombat(); }
...
void PFBaseCreep::SetWarfogFaction(NDb::EFaction faction)
{
  if (warfogFaction == faction)
    return;
...
bool PFBaseCreep::Step(float dtInSeconds)
{
  ...
  const bool levelupPaused = GetWorld() && GetWorld()->GetAIWorld() && GetWorld()->GetAIWorld()->IsCreepLevelupPaused();
  if (ShouldGainLevels() && !levelupPaused)
  {
