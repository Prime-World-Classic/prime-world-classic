...
{
  const char * originalAddr = _reply.externalAddress.c_str();
  MessageTrace( "LoginClient: Got svc request reply. svcid=%s, req_id=%d, code=%d, addr=%s", 
    _reply.svcId.c_str(), _reply.requestId, (int)_reply.code, originalAddr );

  // Если адрес начинается с цифры (порт без IP) — добавляем IP
  if ( originalAddr[0] >= '0' && originalAddr[0] <= '9' )
  {
    // Адрес без IP — добавляем IP от сервера
    _reply.externalAddress = nstl::string( SERVER_IP_ARRAY[usedServer] ) + ":" + nstl::string( originalAddr );
    MessageTrace( "LoginClient: Added missing IP. original=%s, new=%s", 
      originalAddr, _reply.externalAddress.c_str() );
  }
  else if ( strchr( originalAddr, ':' ) != 0 )
  {
    // Адрес с IP:порт — подменяем IP
    _reply.externalAddress = Network::ReplaceIpInAddress( _reply.externalAddress, SERVER_IP_ARRAY[usedServer] );
    MessageTrace( "LoginClient: Replaced IP. original=%s, new=%s, usedServer=%d", 
      originalAddr, _reply.externalAddress.c_str(), usedServer );
  }
  else
  {
    WarningTrace( "LoginClient: Unknown address format! addr=%s, svcid=%s, req_id=%d", 
      originalAddr, _reply.svcId.c_str(), _reply.requestId );
  }
