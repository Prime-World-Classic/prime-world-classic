...
//	WORD  *pPos   = (WORD*)pCompBytes;	// pos in compressed data
	...
	{
		// 8 bytes per block
		pBlock = reinterpret_cast<SDXTColBlock*>( const_cast<BYTE*>( pCompBytes + j * xblocks * 8 ) );
		...
		for( i=0; i < xblocks; i++, pBlock++ )
		{
			...
			pImPos = pBase + i * 4 + ( j * 4 ) * hdr.dwWidth;
...
//	WORD  *pPos   = (WORD*)pCompBytes;	// pos in compressed data
	...
	for ( j = 0; j < yblocks; ++j )
	{
		...
		pBlock = reinterpret_cast<SDXTColBlock*>( const_cast<BYTE*>( pCompBytes + j * xblocks * 16 ) );
