...
```text
/home/vitaly/p/Prime-World/pw/branches/r1117/Bin/logs/2026.04.23-02.32.28/linux-client-shell.log
```
...
- `Render/RenderInterface.cpp` is no longer only a no-op on the Linux executable path, and the Linux shell now constructs and runs a real `Render::DeviceLostWrapper<PF_Render::Interface>` through the canonical `PF_GameLogic/PFRenderInterface.cpp` file. The Linux bootstrap branch there now owns constrained frame setup plus `Render()` / `Present()` for the shell, but it is still a narrow OpenGL bootstrap implementation. The next real render/backend seam is deeper inside that same file, where the full Windows `PF_Render::Interface` body still assumes the broader Direct3D 9 renderer/runtime stack.
