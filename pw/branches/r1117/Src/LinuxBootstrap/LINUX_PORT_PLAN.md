...
```text
/home/vitaly/p/Prime-World/pw/branches/r1117/Bin/logs/2026.04.22-21.44.07/linux-client-shell.log
```
...
- `Render/RenderInterface.cpp` is no longer only a no-op on the Linux executable path. The next real render/backend seam is now above it: `PF_GameLogic/PFRenderInterface.cpp` and the `PW_Client/Game.cpp` path that constructs `Render::DeviceLostWrapper<PF_Render::Interface>`. The base interface can now clear and present through OpenGL on Linux bootstrap, but the real `PF_Render::Interface` implementation still assumes the full Direct3D 9 renderer/runtime stack.
