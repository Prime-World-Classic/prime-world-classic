...
STDMETHODIMP GLDirect3DDevice9::SetRenderTarget(DWORD RenderTargetIndex, IDirect3DSurface9* pRenderTarget) { 
    if (!pRenderTarget) { glBindFramebuffer(GL_FRAMEBUFFER, 0); g_currentFBO = 0; return D3D_OK; }
    // Very simple FBO support for UI
    GLDirect3DSurface9* surf = (GLDirect3DSurface9*)pRenderTarget;
    if (surf->GetParent()) {
        static GLuint fbo = 0; if (fbo == 0) glGenFramebuffers(1, &fbo);
        glBindFramebuffer(GL_FRAMEBUFFER, fbo);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, surf->GetParent()->GetTex(), 0);
        g_currentFBO = fbo;
    } else {
...
STDMETHODIMP GLDirect3DDevice9::Clear(DWORD Count, CONST D3DRECT* pRects, DWORD Flags, D3DCOLOR Color, float Z, DWORD Stencil) {
    ...
    if (Color == 0 && g_currentFBO == 0) { float t = SDL_GetTicks() / 1000.0f; r = 0.5f + 0.4f * sinf(t); g = 0.5f + 0.4f * sinf(t * 1.3f); b = 0.6f + 0.3f * sinf(t * 1.7f); a = 1.0f; }
...
void GLDirect3DDevice9::UpdateShaderProgram() {
    ...
    if (uiCoefsLoc != -1) glUniform4fv(uiCoefsLoc, 1, (float*)&m_vsConstF[23]);
...
STDMETHODIMP GLDirect3DTexture9::LockRect(UINT Level, D3DLOCKED_RECT* pLockedRect, CONST RECT* pRect, DWORD Flags) { if (pLockedRect) { UINT bpp = GetFormatBPP(m_format); pLockedRect->Pitch = m_width * bpp; pLockedRect->pBits = m_pData; } return D3D_OK; }
