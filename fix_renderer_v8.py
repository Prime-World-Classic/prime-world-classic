...
STDMETHODIMP GLDirect3DDevice9::CreateRenderTarget(UINT Width, UINT Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample, DWORD MultisampleQuality, BOOL Lockable, IDirect3DSurface9** ppSurface, HANDLE* pSharedHandle) { if (ppSurface) { *ppSurface = new GLDirect3DSurface9(); } return D3D_OK; }
...
GLDirect3DTexture9::GLDirect3DTexture9(UINT Width, UINT Height, UINT Levels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool) : m_tex(0), m_width(Width), m_height(Height), m_format(Format), m_levels(Levels), m_pData(NULL) { m_refCount = 1; m_pData = malloc(Width * Height * 4); glGenTextures(1, &m_tex); glBindTexture(GL_TEXTURE_2D, m_tex); glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR); }
GLDirect3DTexture9::~GLDirect3DTexture9() { if (m_pData) free(m_pData); if (m_tex) glDeleteTextures(1, &m_tex); }
STDMETHODIMP GLDirect3DTexture9::LockRect(UINT Level, D3DLOCKED_RECT* pLockedRect, CONST RECT* pRect, DWORD Flags) { if (pLockedRect) { pLockedRect->Pitch = m_width * 4; pLockedRect->pBits = m_pData; } return D3D_OK; }
...
STDMETHODIMP GLDirect3DTexture9::GetLevelDesc(UINT Level, D3DSURFACE_DESC *pDesc) { if (pDesc) { pDesc->Format = m_format; pDesc->Type = D3DRTYPE_SURFACE; pDesc->Width = m_width; pDesc->Height = m_height; } return D3D_OK; }
STDMETHODIMP GLDirect3DTexture9::GetSurfaceLevel(UINT Level, IDirect3DSurface9** ppSurfaceLevel) { if (ppSurfaceLevel) { *ppSurfaceLevel = new GLDirect3DSurface9(this, Level); } return D3D_OK; }

GLDirect3DSurface9::GLDirect3DSurface9(GLDirect3DTexture9* pParent, UINT level) : m_pParent(pParent), m_level(level) { m_refCount = 1; }
STDMETHODIMP GLDirect3DSurface9::GetDesc(D3DSURFACE_DESC *pDesc) { 
    if (m_pParent) return m_pParent->GetLevelDesc(m_level, pDesc);
    if (pDesc) { pDesc->Format = D3DFMT_A8R8G8B8; pDesc->Type = D3DRTYPE_SURFACE; pDesc->Width = 1024; pDesc->Height = 768; } 
    return D3D_OK; 
}
